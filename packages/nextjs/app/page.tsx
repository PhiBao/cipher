...
const PillButton = ({
  ...
  const base =
    "inline-flex items-center justify-center transition-transform duration-150 active:scale-95 disabled:opacity-40 disabled:pointer-events-none";
  const styles =
    variant === "primary"
      ? "bg-apple-blue text-white rounded-full px-6 py-3 text-body"
      : variant === "secondary"
        ? "bg-transparent text-apple-blue border border-apple-blue rounded-full px-6 py-3 text-body"
...
const SectionTitle = ({ title, subtitle }: { title: string; subtitle?: string }) => (
  <div className="text-center mb-12">
    ...
    {subtitle && <p className="text-lead text-white/80 max-w-2xl mx-auto">{subtitle}</p>}
  </div>
);
...
const InfoTooltip = ({ text }: { text: string }) => (
  <span className="group relative inline-block ml-1">
    <svg width="14" height="14" viewBox="0 0 14 14" fill="none" className="inline text-white/40">
      <circle cx="7" cy="7" r="6.5" stroke="currentColor" />
      <path d="M7 4V5.5M7 6.5V10" stroke="currentColor" strokeLinecap="round" />
    </svg>
    <span className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-48 bg-apple-ink text-white text-xs rounded-lg px-3 py-2 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-50">
      {text}
    </span>
  </span>
);
...
const ApplyPanel = ({ cipher }: { cipher: ReturnType<typeof useCipherProtocolWagmi> }) => {
  ...
  return (
    <div className="bg-apple-tile-2 rounded-lg p-6 md:p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        <div>
          <label className="text-caption text-white/60 block mb-2">
            Transaction Count
            <InfoTooltip text="Total unique transactions on Sepolia. Read-only from Alchemy." />
          </label>
        ...
        <div>
          <label className="text-caption text-white/60 block mb-2">
            Total Volume (ETH)
            <InfoTooltip text="Sum of all ETH transferred in + out. Read-only from Alchemy." />
          </label>
        ...
        <div>
          <label className="text-caption text-white/60 block mb-2">
            Wallet Age (days)
            <InfoTooltip text="Days since first transaction. Read-only from Alchemy." />
          </label>
      ...
      <p className="text-caption text-white/40 mt-3">
        Your data is encrypted in-browser before reaching the blockchain. The contract never sees raw values.
      </p>
    </div>
  );
};
...
const ScorePanel = ({ cipher }: { cipher: ReturnType<typeof useCipherProtocolWagmi> }) => {
  ...
  return (
    <div className="bg-apple-tile-2 rounded-lg p-6 md:p-8">
      <h3 className="text-tagline text-white mb-2">My Credit Score</h3>
...
const LoanPanel = ({ cipher }: { cipher: ReturnType<typeof useCipherProtocolWagmi> }) => {
  ...
  const [repayAmount, setRepayAmount] = useState("0.05");

  const tierName = cipher.userTier > 0 ? ["", "A", "B", "C", "D"][cipher.userTier] : null;
  const limitEth = cipher.tierLimit > 0 ? formatEther(cipher.tierLimit) : "0";

  return (
    <div className="bg-apple-tile-2 rounded-lg p-6 md:p-8">
      <h3 className="text-tagline text-white mb-6">Micro-Lending</h3>
      ...
      {!tierName ? (
        ...
        <>
          ...
          <div className="space-y-1 mb-6">
            ...
            {cipher.loanActive && <DataRow label="Repaid" value={fmtEth(cipher.loanRepaid)} />}
          </div>
          ...
          {cipher.loanActive && (
            <div className="flex flex-col sm:flex-row gap-3">
              <input
                ...
                className="flex-1 bg-apple-tile-3 text-white rounded-lg px-4 py-3 text-body border border-white/10 focus:border-apple-blue focus:outline-none"
              />
              <PillButton onClick={() => cipher.repayLoan(repayAmount)} disabled={cipher.isProcessing}>
                {cipher.isProcessing ? "Processing..." : "Repay Loan"}
              </PillButton>
            </div>
          )}
        </>
...
const PoolPanel = ({ cipher }: { cipher: ReturnType<typeof useCipherProtocolWagmi> }) => {
  ...
  return (
    <div className="bg-apple-canvas border border-apple-hairline rounded-lg p-6 md:p-8">
      <h3 className="text-tagline text-apple-ink mb-6">Lending Pool</h3>
      <div className="space-y-1 mb-6">
        <div className="flex justify-between items-center py-3 border-b border-apple-divider">
          <span className="text-caption text-apple-ink-muted-48">Pool Balance</span>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <PillButton onClick={() => cipher.depositLiquidity(depositAmount)} disabled={cipher.isProcessing}>
          {cipher.isProcessing ? "Processing..." : "Deposit Liquidity"}
        </PillButton>
      </div>
      <p className="text-caption text-apple-ink-muted-48 mt-3">
        Depositors earn yield from borrower repayments. Funds are non-custodial.
      </p>
    </div>
  );
};
    ...
    <div className="flex flex-col">
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              ...
              <div key={item.step} className="bg-apple-canvas rounded-lg border border-apple-hairline p-6 md:p-8">
      ...
      <section id="dashboard" className="bg-apple-tile-1 py-20 md:py-28 px-6">
        <div className="max-w-5xl mx-auto">
          ...
          {!isConnected ? (
            <div className="bg-apple-tile-2 rounded-lg p-12 text-center">
              <div className="w-16 h-16 rounded-full bg-white/5 flex items-center justify-center mx-auto mb-4">
            ...
            <div className="space-y-6">
              {cipher.message && (
                <div className="bg-apple-blue/10 border border-apple-blue/30 rounded-lg px-6 py-4 flex items-start gap-3">
                  <svg
                    width="20"
                    height="20"
                    viewBox="0 0 20 20"
      ...
      {/* Pool Stats */}
      <section id="pool" className="bg-apple-canvas py-20 md:py-28 px-6">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-16">
            <h2 className="text-display-lg text-apple-ink mb-3">Protocol Stats</h2>
            <p className="text-body text-apple-ink-muted-48 max-w-xl mx-auto">
              Transparent pool metrics. Individual positions remain confidential.
            </p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
            {[
              { label: "Pool Liquidity", value: fmtEth(cipher.poolBalance) },
              { label: "Max Loan (Tier A)", value: "10 ETH" },
              { label: "Min Loan (Tier D)", value: "0.5 ETH" },
            ].map(stat => (
              <div key={stat.label} className="bg-apple-parchment rounded-lg p-8 text-center">
                <p className="text-caption text-apple-ink-muted-48 mb-2">{stat.label}</p>
                <p className="text-display-md text-apple-ink">{stat.value}</p>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="space-y-4">
            {[
              ...
              },
              {
                ...
                desc: "Community-funded ETH pool. Borrowers access tier-based loans after proving their creditworthiness.",
              },
            ].map((item, i) => (
              <div
                key={i}
                className="bg-apple-canvas rounded-lg border border-apple-hairline p-6 md:p-8 flex flex-col md:flex-row md:items-start gap-4"
              >
